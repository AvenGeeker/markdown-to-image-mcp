...
async function runComprehensiveTest() {
  console.log('Starting comprehensive MCP Markdown-to-Image Server test...');
...
*Powered by MCP Markdown-to-Image Service*`;
    ...
    // Test different configurations including templates
    const testConfigs = [
      {
        name: 'Default Blue Theme with QuoteCard Template',
        options: {
          theme: 'blue',
          template: 'QuoteCard',
          size: 'mobile',
      ...
      {
        name: 'Spring Gradient Wave Theme with NewsDigest Template',
        options: {
      ...
      {
        name: 'Purple Square Format with QuoteCard Template',
        options: {
          theme: 'purple',
          template: 'QuoteCard',
          aspectRatio: '1/1',
      ...
      {
        name: 'Green 16/9 Format with NewsDigest Template',
        options: {
          theme: 'green',
          template: 'NewsDigest',
          aspectRatio: '16/9',
    ...
    for (const config of testConfigs) {
      ...
      try {
        ...
        if (result.success && result.imageData) {
          const filename = `test-${config.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}.${config.options.format}`;
    ...
    // Test QuoteCard template
    const previewQuoteCard = await previewTool.execute({
      markdown: '# Test Preview\n\nThis is a **test** preview with QuoteCard template.',
      theme: 'blue',
      template: 'QuoteCard'
    });
    console.log('✓ QuoteCard template preview generation successful');
    
    // Test NewsDigest template
    const previewNewsDigest = await previewTool.execute({
      markdown: '# Test Preview\n\nThis is a **test** preview with NewsDigest template.',
      theme: 'SpringGradientWave',
      template: 'NewsDigest'
    });
    console.log('✓ NewsDigest template preview generation successful');
